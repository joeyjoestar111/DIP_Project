...
class DM2FNet(Base):
    def __init__(self, num_features=128, arch='resnext101_32x8d'):
        ...
        self.attention_fusion = nn.Sequential(
            ...
            nn.Conv2d(num_features, num_features // 2, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features // 2, num_features // 2, kernel_size=3, padding=1), nn.SELU(),
            nn.Conv2d(num_features // 2, 21, kernel_size=1)
        )
    ...
    def forward(self, x0, x0_hd=None):
        ...
        r6 = F.upsample(self.j5(f5), size=x0.size()[2:], mode='bilinear')
        x_j6 = ((torch.exp(x0 + r6) - torch.exp(- (x0 + r6))) / 2 * self.std + self.mean).clamp(min=0., max=1.)

        attention_fusion = F.upsample(self.attention_fusion(concat), size=x0.size()[2:], mode='bilinear')
        x_f0 = torch.sum(F.softmax(attention_fusion[:, :7, :, :], 1) *
                         torch.stack((x_phy[:, 0, :, :], x_j1[:, 0, :, :], x_j2[:, 0, :, :],
                                      x_j3[:, 0, :, :], x_j4[:, 0, :, :], x_j5[:, 0, :, :], x_j6[:, 0, :, :]), 1), 1,
                         True)
        x_f1 = torch.sum(F.softmax(attention_fusion[:, 7: 14, :, :], 1) *
                         torch.stack((x_phy[:, 1, :, :], x_j1[:, 1, :, :], x_j2[:, 1, :, :],
                                      x_j3[:, 1, :, :], x_j4[:, 1, :, :], x_j5[:, 1, :, :], x_j6[:, 0, :, :]), 1), 1,
                         True)
        x_f2 = torch.sum(F.softmax(attention_fusion[:, 14:, :, :], 1) *
                         torch.stack((x_phy[:, 2, :, :], x_j1[:, 2, :, :], x_j2[:, 2, :, :],
                                      x_j3[:, 2, :, :], x_j4[:, 2, :, :], x_j5[:, 2, :, :], x_j6[:, 0, :, :]), 1), 1,
                         True)
        x_fusion = torch.cat((x_f0, x_f1, x_f2), 1).clamp(min=0., max=1.)

        if self.training:
            return x_fusion, x_phy, x_j1, x_j2, x_j3, x_j4, x_j5, x_j6, t, a.view(x.size(0), -1)
        else:
            return x_fusion
...
class SimpleAttention(nn.Module):
    def __init__(self, input_channels):
        ...
        self.attention_fc = nn.Linear(input_channels, 3)  # 输出三个权重

    def forward(self, out1, out2, out3):
        # 计算每个输出张量的全局特征
        out1_mean = out1.mean(dim=(2, 3))
        out2_mean = out2.mean(dim=(2, 3))
        out3_mean = out3.mean(dim=(2, 3))

        # 将特征拼接在一起
        combined_features = torch.cat([out1_mean, out2_mean, out3_mean], dim=1)
        ...
        # 重新调整权重的形状
        attention_weights = attention_weights.unsqueeze(2).unsqueeze(3).unsqueeze(4)

        # 对三个张量进行加权求和
        weighted_sum = (attention_weights[:, 0, :, :, :] * out1 +
                        attention_weights[:, 1, :, :, :] * out2 +
                        attention_weights[:, 2, :, :, :] * out3)
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=3, init_features=32):
        ...
        self.attention = SimpleAttention(input_channels=3 * 3)
    ...
    def forward(self, x):
        ...
        concat = self.attention(out_1, out_2, out_3)
...
class DM2FNet_woPhy(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        ...
        # self.down0 = UNet(in_channels = 64, out_channels = num_features)
        # self.down1 = UNet(in_channels = 256, out_channels = num_features)
        # self.down2 = UNet(in_channels = 512, out_channels = num_features)
        # self.down3 = UNet(in_channels = 1024, out_channels = num_features)
        # self.down4 = UNet(in_channels = 2048, out_channels = num_features)
